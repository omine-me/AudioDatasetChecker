import ModuleFactory from './_hdf5_util.js';
    ...
    *items() {
    ...
    create_dataset(args) {
        ...
        if (metadata.vlen) {
            ...
            try {
                Module.HEAPU8.set(prepared_data, data_ptr);
                Module.create_dataset(this.file_id, this.path + "/" + name, BigInt(data_ptr), final_shape, final_maxshape, final_chunks, metadata.type, metadata.size, metadata.signed, metadata.vlen, compression_id, compression_opts_out);
            }
            finally {
                Module._free(data_ptr);
            }
